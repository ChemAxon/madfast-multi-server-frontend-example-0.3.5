...
if curl --output /dev/null --silent --head --fail "http://localhost:$SRV1_PORT/rest/statistics" ; then
    echo "Server 1 alive (on port $SRV1_PORT)"
else
    echo "Server 1 not available"
fi

if curl --output /dev/null --silent --head --fail "http://localhost:$SRV2_PORT/rest/statistics" ; then
    echo "Server 2 alive (on port $SRV2_PORT)"
else
    echo "Server 2 not available"
fi

if curl --output /dev/null --silent --head --fail "http://localhost:$SRV3_PORT/rest/statistics" ; then
    echo "Server 3 alive (on port $SRV3_PORT)"
else
    echo "Server 3 not available"
fi

if curl --output /dev/null --silent --head --fail "http://localhost:$SRV4_PORT/rest/statistics" ; then
